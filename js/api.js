class GerenciadorEquipamentos {
  constructor() {
    this.apiUrl = "http://localhost:5144/api";
    this.equipamentos = [];
  ...
  mostrarErro(campo, mensagem) {
    let errorEl = document.getElementById(`${campo}Error`);
    ...
    tbody.innerHTML = this.previewImportacao
      .map(
        ...
        <tr>
          ...
          <td>${e.modelo}</td>
          <td>${e.geracao}</td>
          <td>R$ ${Number(e.valorAluguel).toFixed(2)}</td>
  ...
  renderListagem() {
    ...
    const equipamentosOrdenados = this.ordenarEquipamentos(equipamentosFiltrados);
  ...
  aplicarFiltros() {
    ...
    return this.equipamentos.filter((eq) => {
      return (
        (!tag || eq.tag.toString().toUpperCase().includes(tag)) &&
  ...
  formatarData(data) {
    if (!data || data.startsWith("0001")) return "Não cadastrado";
  ...
  criarLinhaTabela(equip) {
    ...
    row.innerHTML = `
      ...
      <td>${equip.setor}</td>
      <td>${equip.usuario || "-"}</td>
      <td>${equip.dataChegada ? this.formatarData(equip.dataChegada) : "Não cadastrado"}</td>
      <td>${equip.modelo || "-"}</td>
      <td>${equip.geracao || "-"}</td>
      <td>
      ...
      <td>R$ ${Number(equip.valorAluguel).toFixed(2).replace(".", ",")}</td>
      <td>
      ...
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <select id="editSetor">
            ...
            <option value="Planejamento" ${equip.setor === "Planejamento" ? "selected" : ""}>Planejamento</option>
  ...
  async mostrarHistorico(id) {
    try {
      const historico = await this.apiRequest(`/equipamentos/${id}/historico`);
      if (!historico) return;

      const equip = this.equipamentos.find((e) => e.id == id);
      if (!equip) return;

      document.getElementById("modalTitle").textContent = `Histórico - ${equip.tag}`;
            ...
            <div class="historico-item">
              ...
              <div class="historico-detalhes">
                ${
                  h.acao === "Entrada"
                    ? `
                      <strong>Status:</strong> Equipamento entrou no estoque<br>
                      <strong>Usuário vinculado:</strong> ${h.usuario || "-"}<br>
                    `
                    : ""
                }
                ...
                ${
                  h.acao === "Saída"
                    ? `
                      <strong>Status:</strong> Equipamento saiu do estoque<br>
                      <strong>Data de saída:</strong> ${this.formatarData(h.data)}<br>
                      <strong>Último usuário:</strong> ${h.usuario || "-"}<br>
                    `
                    : ""
                }
                ...
                ${
                  h.acao === "Editado"
                    ? `
                      <strong>Usuário:</strong> ${antigos?.Usuario || "-"} → ${novos?.Usuario || "-"}<br>
                      <strong>Setor:</strong> ${antigos?.Setor || "-"} → ${novos?.Setor || "-"}<br>
                      <strong>Status:</strong> ${novos?.Ativo ? "Ativo" : "Inativo"}<br>
                    `
                    : ""
                }
              </div>
  ...
  renderizarGraficoSetores(dados) {
    ...
    if (total === 0) {
      container.innerHTML = "<p>Nenhum equipamento ativo.</p>";
      return;
    }
